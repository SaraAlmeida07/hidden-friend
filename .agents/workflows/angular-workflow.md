---
description: Configurar a fundação visual do Monorepo utilizando Angular 21 (Zoneless/Signals), Tailwind CSS v4 (CSS-First) e o Spartan UI.
---
...
## 🧩 Bloco 3.4: Setup do Spartan UI (Opcional)

**Explicação Didática:** O Spartan UI é uma biblioteca de componentes headless para Angular, construída sobre primitives acessíveis. Diferente do DaisyUI, ele não entrega apenas classes CSS, mas componentes Angular reutilizáveis e alinhados com o ecossistema moderno (Standalone Components, Signals e Tailwind CSS v4).

### Ação de Instalação (Via Expressa)

Apresente ao aluno o comando oficial de instalação do Spartan UI no projeto Angular:
...
> - Configurar o Tailwind CSS v4;
> - Criar o arquivo `components.json`;
> - Preparar o projeto para geração de componentes.

📘 Documentação oficial: [Spartan UI Installation Guide](https://spartan.ng/documentation/installation?utm_source=chatgpt.com)

### Ação de Geração do Primeiro Componente

Após a instalação, gere o componente `button`:

```bash
npx ng g @spartan-ng/cli:ui button
```

### Ação de Teste Visual

Altere temporariamente o `app.component.html` para validar a renderização do componente:

```html
<div class="flex h-screen items-center justify-center bg-zinc-950">
  <button hlmBtn variant="destructive">Teste Spartan UI</button>
</div>

<router-outlet />
```

> O atributo `hlmBtn` é a diretiva do componente Button do Spartan.

---

🛑 **PARADA OBRIGATÓRIA (Smoke Test Final):**

- **Ação do Aluno:** Abra o navegador em `http://localhost:4200`.
- **Pergunta:**
  _"O botão estilizado pelo Spartan UI apareceu centralizado na tela? Se sim, o Design System base do projeto está homologado e totalmente alinhado ao SDD."_
